...
homeserver (example.com via Synapse + MAS) to claude.ai as a custom
connector. Single binary, self-hosted on Gruyere at
`matrix-mcp.example.com`. OAuth via MAS (no new user system).
...
6. Token introspection validates `resource=https://matrix-mcp.example.com`
   (RFC 8707)
...
- Real: add `https://matrix-mcp.example.com` to claude.ai → OAuth
...
- Plaintext test room on example.com, send messages from Element,
...
**Goal:** live at `matrix-mcp.example.com`.
...
- **Integration tests:** mock MAS introspection in Phase 1; real Matrix
  client against `@matrix-mcp-dev:example.com` (test account, to be
  created) in Phase 2+.
