...
matrix-mcp ─(2)── 401 + WWW-Authenticate: resource_metadata=https://matrix-mcp.example.com/.well-known/oauth-protected-resource ──► claude.ai
claude.ai ─(3)── GET /.well-known/oauth-protected-resource                                    ──► matrix-mcp
matrix-mcp ─(4)── {authorization_servers: ["https://matrixauthservice.example.com"]}      ──► claude.ai
claude.ai ─(5)── GET https://matrixauthservice.example.com/.well-known/oauth-authorization-server ──► MAS
...
claude.ai ─(8)── POST /oauth2/token  (code, verifier, resource=https://matrix-mcp.example.com) ──► MAS
...
MAS ──── {active: true, sub: @alice:example.com, scope: ..., device_id: matrix-mcp-XXXX, aud: https://matrix-mcp.example.com} ──► matrix-mcp
