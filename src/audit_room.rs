...
mod tests {
    ...
    fn audit_room_content_round_trips_json() {
        let original = AuditRoomEventContent {
            room_id: "!audit:kampong.social".to_owned(),
        };
        let json = serde_json::to_value(&original).unwrap();
        assert_eq!(json["room_id"], "!audit:kampong.social");
    ...
    fn notice_body_is_envelope_only_with_room() {
        ...
        let target = Some("!foo:kampong.social".to_owned());
        let outcome = "ok";
        ...
        assert_eq!(
            body,
            "matrix-mcp: send_text_message \u{2192} !foo:kampong.social outcome=ok"
        );
