...
impl HomeserverClient {
    ...
    /// trailing slash, e.g. `https://matrix.kampong.social`.
...
mod tests {
    ...
    async fn joined_rooms_round_trip() {
        ...
        assert_eq!(rooms, vec!["!abc:kampong.social", "!def:kampong.social"]);
    }
    ...
    async fn recent_messages_pulls_with_dir_b() {
        ...
        let client = HomeserverClient::new(server.uri()).unwrap();
        let events = client
            .recent_messages("user-token", "!abc:kampong.social", 5)
            .await
            .unwrap();
    ...
    async fn send_text_message_returns_event_id() {
        ...
        let client = HomeserverClient::new(server.uri()).unwrap();
        let id = client
            .send_text_message("user-token", "!abc:kampong.social", "Hello, world!")
            .await
            .unwrap();
    ...
    fn urlencode_escapes_matrix_id_special_chars() {
        assert_eq!(urlencode("!abc:kampong.social"), "%21abc%3Akampong.social");
