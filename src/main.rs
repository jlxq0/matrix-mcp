...
mod tests {
    ...
    async fn list_joined_rooms_tool_round_trips() {
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({
                "joined_rooms": ["!a:example.com", "!b:example.com"]
            })))
            .mount(&mock)
            .await;
        ...
        assert_eq!(room_ids[0], "!a:example.com");
    }
    ...
    async fn read_recent_messages_tool_round_trips() {
        ...
        let (status, body) = initialize_then_call_tool(
            ...
            json!({"room_id": "!abc:example.com", "limit": 5}),
        )
        .await
        .unwrap();
        assert_eq!(status, StatusCode::OK, "body: {body}");
    ...
    async fn send_text_message_tool_returns_event_id() {
        ...
        let (status, body) = initialize_then_call_tool(
            &app,
            "send_text_message",
            json!({"room_id": "!abc:example.com", "body": "Hello, world!"}),
        )
        .await
        .unwrap();
        assert_eq!(status, StatusCode::OK, "body: {body}");
